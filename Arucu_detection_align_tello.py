    ...
    while True:
        ...
        if i % 3 == 0:  # display only one third of the frames, you can change this parameter according to your needs
           # ret, img = frame_read.frame
            ing = drone.get_frame_read().frame
           # img = frame_read.frame
            ...
            if ids is not None:
                ...
                top_position = math.degrees(yaw_camera)
                if tvec[1] > -35:
                    cv2.putText(img, f"ID: {ids[0][0]} Y position accurate, move down!", (20, 20), cv2.FONT_HERSHEY_COMPLEX, 0.5,
                                (255, 255, 255), 2, cv2.LINE_AA)
                if math.degrees(yaw_camera) > 10 or math.degrees(yaw_camera) < -10:
                    if math.degrees(yaw_camera) > 10 or math.degrees(yaw_camera) > -10:
                        drone_rotate(drone, math.degrees(yaw_camera))
                    if not math.degrees(yaw_camera) > 10 or not math.degrees(yaw_camera) > -10:
                        # drone.send_rc_control(0,0,0,0)
                        cv2.putText(img, f"ID: {ids[0][0]} rotate to the right!", (20, 40),
                                    cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
                    else:
                        cv2.putText(img, f"ID: {ids[0][0]} rotate to the left!", (20, 40),
                                    cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA)

                if tvec[0] > 10 or tvec[0] > -10:
                    drone_maintain_x(drone, tvec[0])
                if not tvec[0] > 10 or not tvec[0] > -10:
                    # drone.send_rc_control(0, 0, 0, 0)
                    pass

                if tvec[1] > 10 or tvec[1] > -10:
                    drone_maintain_y(drone, tvec[1])
                if not tvec[1] > 10 or not tvec[1] > -10:
                    # drone.send_rc_control(0, 0, 0, 0)
                    pass
